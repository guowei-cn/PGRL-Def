...
@torch.no_grad()
def evaluating(model, ts_dl, epoch, device, writer, poison_flag=False, normalization=None, net_G=None, target_class=None):
    pred_l, gt_l, loss_l = [], [], []
    model.eval()
    for batch in ts_dl:
...
def get_optimizer(model, poison_type, model_name='cnn'):
    if poison_type == 'adaptivecifar10' or poison_type == 'blto' or poison_type== 'pattern' \
        ...
        if model_name == 'cnn':
            ...
            print(model_name)
            epoch_num, lr, momentum = 50, 0.01, 0.99
            optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=momentum)
            schedule = torch.optim.lr_scheduler.MultiStepLR(optimizer, [100, 130, 150], 0.1)
        else:
...
def main(args):
    ...
    optimizer, epoch_num, schedule = get_optimizer(model, poison_type, model_name=model_name)
    ...
    for epoch in range(1, epoch_num+1):
        batch_id, loss_v_l, poison_flag_l = training(model, tr_dl, non_shuffle_tr_dl, criterion, optimizer, epoch, device, writer, batch_id)
    ...
    torch.save(model.state_dict(), 'poisonDataset/{}/train_{}_{}_pr_{}_mn_{}.pth'.format(poison_type, poison_or_benign, num_class, poison_rate, model_name))
